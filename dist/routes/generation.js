...
router.post('/generate', async (req, res) => {
    try {
        ...
        processGeneration(generation._id.toString());
        res.json({
            success: true,
...
router.get('/status/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const generation = await Generation_1.Generation.findById(id);
        if (!generation) {
...
router.get('/history', async (req, res) => {
    try {
        ...
        const generations = await Generation_1.Generation.find()
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(Number(limit))
            .select('-githubToken'); // Exclude sensitive data
        const total = await Generation_1.Generation.countDocuments();
        res.json({
            success: true,
            generations,
            pagination: {
...
router.post('/push-dockerfile', async (req, res) => {
    try {
        ...
        const generation = await Generation_1.Generation.findById(generationId);
        if (!generation) {
            return res.status(404).json({
                error: 'Generation not found'
            });
        ...
        const githubService = new GitHubService_1.GitHubService(generation.githubToken);
        const success = await githubService.pushDockerfileToRepository(generation.githubUrl, generation.dockerfile, commitMessage || 'Add Dockerfile generated by DockGen AI');
        if (success) {
            res.json({
                success: true,
...
router.get('/images', async (req, res) => {
    try {
        const dockerService = new DockerService_1.DockerService();
...
async function processGeneration(generationId) {
    try {
        ...
        const aiAgentService = new AIAgentService_1.AIAgentService();
